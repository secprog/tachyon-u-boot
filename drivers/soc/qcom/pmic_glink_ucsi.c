...
int qpg_ucsi_discover(struct qpg *pg)
{
	...
	log_warning("pmic-glink: UCSI discover begin (post-PAN)\n");

	ret = qpg_send_ucsi_ppm_reset(pg);
	log_warning("pmic-glink: UCSI discover PPM_RESET ret=%d\n", ret);
	if (ret)
		return ret;

	ret = qpg_enable_ucsi_notifications(pg);
	log_warning("pmic-glink: UCSI discover notifications ret=%d\n", ret);
	if (ret)
		return ret;

	ret = qpg_send_ucsi_get_capability(pg);
	log_warning("pmic-glink: UCSI discover capability ret=%d\n", ret);
	if (ret)
		return ret;

	ret = qpg_send_ucsi_get_connector_capability(pg, 0);
	log_warning("pmic-glink: UCSI discover connector capability ret=%d\n",
		    ret);
	if (ret)
		return ret;

	ret = qpg_send_ucsi_get_connector_status(pg, 0);
	log_warning("pmic-glink: UCSI discover connector status ret=%d\n", ret);
	if (ret)
		return ret;

	ret = qpg_send_ucsi_get_cam_supported(pg, 0);
	log_warning("pmic-glink: UCSI discover CAM_SUPPORTED ret=%d\n", ret);
	if (ret)
		return ret;

	ret = qpg_send_ucsi_get_current_cam(pg, 0);
	log_warning("pmic-glink: UCSI discover CURRENT_CAM ret=%d\n", ret);
	if (ret)
		return ret;

	ret = qpg_send_ucsi_get_alternate_mode(pg, 0, 0, 2);
	log_warning("pmic-glink: UCSI discover ALT_MODE ret=%d\n", ret);
	if (ret)
		return ret;

	ret = qpg_enable_ucsi_notifications_phase2(pg);
	log_warning("pmic-glink: UCSI discover notifications phase2 ret=%d\n",
		    ret);
